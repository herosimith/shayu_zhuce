...
if (document.documentElement.getAttribute(SIGNUP_PAGE_LISTENER_SENTINEL) !== '1') {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (
      ...
      || message.type === 'WAIT_STEP5_DELAYED_ONBOARDING'
...
async function handleCommand(message) {
  switch (message.type) {
    ...
    case 'ENSURE_SIGNUP_ENTRY_READY':
      return await ensureSignupEntryReady(Math.max(1000, Number(message.payload?.timeoutMs) || 15000));
    ...
    case 'WAIT_STEP5_DELAYED_ONBOARDING':
      return await waitForStep5DelayedOnboardingCommand(message.payload || {});
...
const MFA_CHALLENGE_PAGE_PATTERN = /验证你的身份|验证您的身份|查看你首选的一次性密码程序|一次性验证码|authenticator|two[-\s]*factor|multi[-\s]*factor|mfa|one[-\s]*time\s+(?:password|passcode|code)|verification\s+code/i;
...
function isMfaChallengePage() {
  ...
  if (/\/mfa-challenge(?:[/?#]|$)/i.test(path)) return true;
  const pageText = getPageTextSnapshot();
  return Boolean(getVerificationCodeTarget())
    && MFA_CHALLENGE_PAGE_PATTERN.test(pageText)
...
const COOKIE_CONSENT_CONTEXT_PATTERN = /cookie|cookies|\u6211\u4eec\u4f7f\u7528\s*cookie|cookie\s*\u53ef\u5e2e\u52a9|\u504f\u597d\u8bbe\u7f6e|cookie\s+policy|privacy\s+preferences/i;
const COOKIE_CONSENT_ACTION_PATTERN = /^(?:\u5168\u90e8\u63a5\u53d7|\u63a5\u53d7\u5168\u90e8|\u63a5\u53d7\u6240\u6709|\u62d2\u7edd\u975e\u5fc5\u9700|\u62d2\u7edd\u975e\u5fc5\u8981|\u4ec5\u5fc5\u9700|\u63a5\u53d7|\u62d2\u7edd|accept\s+all|reject\s+non[-\s]*(?:essential|necessary)|reject\s+all|decline\s+(?:optional|non[-\s]*essential))$/i;
const COOKIE_CONSENT_PREFERRED_ACTION_PATTERN = /^(?:\u62d2\u7edd\u975e\u5fc5\u9700|\u62d2\u7edd\u975e\u5fc5\u8981|\u4ec5\u5fc5\u9700|\u62d2\u7edd|reject\s+non[-\s]*(?:essential|necessary)|reject\s+all|decline\s+(?:optional|non[-\s]*essential))$/i;
const COOKIE_CONSENT_CLOSE_PATTERN = /^(?:\u5173\u95ed|\u5173\u6389|close|dismiss)$/i;

function getElementContextText(el) {
  const container = el?.closest?.('[role="dialog"], [aria-modal="true"], dialog, [data-testid*="modal" i], [class*="modal" i]');
  const textSource = container || document.body || el;
  return String(textSource?.innerText || textSource?.textContent || '')
    .replace(/\s+/g, ' ')
    .trim();
}

function dismissSignupCookieConsentIfPresent() {
  const candidates = Array.from(document.querySelectorAll('button, [role="button"]'));
  const visibleActions = candidates.filter((el) => isVisibleElement(el) && isActionEnabled(el));
  const action = visibleActions.find((el) => {
    const text = getActionText(el);
    return COOKIE_CONSENT_PREFERRED_ACTION_PATTERN.test(text) && COOKIE_CONSENT_CONTEXT_PATTERN.test(getElementContextText(el));
  }) || visibleActions.find((el) => {
    const text = getActionText(el);
    return COOKIE_CONSENT_ACTION_PATTERN.test(text) && COOKIE_CONSENT_CONTEXT_PATTERN.test(getElementContextText(el));
  }) || visibleActions.find((el) => {
    const text = getActionText(el);
    return COOKIE_CONSENT_ACTION_PATTERN.test(text);
  }) || visibleActions.find((el) => {
    const text = getActionText(el);
    return COOKIE_CONSENT_CLOSE_PATTERN.test(text) && COOKIE_CONSENT_CONTEXT_PATTERN.test(getElementContextText(el));
  });
  if (!action) {
    return false;
  }
...
function isSignupCookieConsentVisible() {
  return Array.from(document.querySelectorAll('button, [role="button"]')).some((el) => {
    if (!isVisibleElement(el)) return false;
    const text = getActionText(el);
    return (COOKIE_CONSENT_ACTION_PATTERN.test(text) || COOKIE_CONSENT_CLOSE_PATTERN.test(text))
      && COOKIE_CONSENT_CONTEXT_PATTERN.test(getElementContextText(el));
  });
}
...
function inspectSignupEntryState() {
  if (isSignupCookieConsentVisible()) {
    return {
...
async function waitForSignupEntryState(options = {}) {
  ...
  while (Date.now() - start < timeout) {
    throwIfStopped();
    if (dismissSignupCookieConsentIfPresent()) {
      await sleep(300);
      continue;
    }
    if (await recoverAuthLoginProblemPageIfPresent({ step })) {
      continue;
    }
...
async function ensureSignupEntryReady(timeout = 15000) {
  ...
  if (
    snapshot.state === 'entry_home'
    || snapshot.state === 'phone_entry'
    || snapshot.state === 'email_entry'
    || snapshot.state === 'password_page'
    || snapshot.state === 'verification_page'
    || snapshot.state === 'phone_verification_page'
...
async function fillSignupEmailAndContinue(email, step) {
  dismissSignupCookieConsentIfPresent();
  const performOperationWithDelay = typeof getOperationDelayRunner === 'function'
    ? getOperationDelayRunner()
    : async (metadata, operation) => {
        const rootScope = typeof window !== 'undefined' ? window : globalThis;
  ...
  if (
    ...
    log(`步骤 ${step}：当前页面已进入验证码或后续阶段，无需重复提交邮箱。`, 'warn');
    return {
      alreadyOnVerificationPage: snapshot.state === 'verification_page',
      alreadyOnPhoneVerificationPage: snapshot.state === 'phone_verification_page',
      alreadyOnProfilePage: snapshot.state === 'profile_page',
      alreadyLoggedInHome: snapshot.state === 'logged_in_home',
  ...
  let emailInput = snapshot.emailInput || getSignupEmailInput();
  if (!emailInput) {
    await sleep(300);
    dismissSignupCookieConsentIfPresent();
    emailInput = getSignupEmailInput();
  }
  if (!emailInput) {
    if (step === 2) {
...
function getAuthLoginProblemReturnButton({ allowDisabled = false } = {}) {
  const pageText = getPageTextSnapshot();
  if (!/糟糕|oops/i.test(pageText) || !/登录时出现问题|problem\s+(?:while\s+)?(?:logging|signing)\s+in|problem\s+with\s+(?:login|sign\s*in)/i.test(pageText)) {
    return null;
  }
  ...
  return Array.from(candidates).find((el) => {
    if (!isVisibleElement(el) || (!allowDisabled && !isActionEnabled(el))) return false;
    return /^(?:返回|back)$/i.test(getActionText(el));
  }) || null;
}

async function recoverAuthLoginProblemPageIfPresent(options = {}) {
  const { step = 2, logLabel = '' } = options;
  const returnButton = getAuthLoginProblemReturnButton();
  if (!returnButton) {
    return false;
  }

  log(logLabel || `步骤 ${step}：检测到登录异常页，正在点击“返回”恢复...`, 'warn');
  await humanPause(350, 900);
  simulateClick(returnButton);
  await sleep(1200);
  return true;
}
...
async function recoverCurrentAuthRetryPage(payload = {}) {
  ...
  const resolvedStepKey = stepKey || (step === 8 || flow === 'login' ? 'oauth-login' : 'fetch-signup-code');
  ...
  while (clickCount < maxClickAttempts) {
    ...
    if (retryState.retryButton && retryState.retryEnabled) {
      ...
      await performOperationWithDelay({ stepKey: resolvedStepKey, kind: 'click', label: 'auth-retry-click' }, async () => {
...
function inspectLoginAuthState() {
  ...
  const mfaChallengePage = isMfaChallengePage();
...
function serializeLoginAuthState(snapshot) {
  return {
    ...
    mfaChallengePage: Boolean(snapshot?.mfaChallengePage),
...
async function prepareSignupVerificationFlow(payload = {}, timeout = 30000) {
  ...
  while (Date.now() - start < timeout && recoveryRound < maxRecoveryRounds) {
    ...
    if (snapshot.state === 'mfa_challenge') {
      await waitForDocumentLoadComplete(15000, `${prepareLogLabel}：2FA 验证页面`);
      await waitForVerificationCodeTarget(15000);
      log(`${prepareLogLabel}：检测到 2FA 验证页面，交由后台获取一次性验证码。`, 'info');
      return {
        ready: true,
        mfaChallenge: true,
        retried: recoveryRound,
...
async function waitForVerificationSubmitOutcome(step, timeout, options = {}) {
  ...
  while (Date.now() - start < resolvedTimeout) {
    ...
    if (step === 4) {
      ...
      if (purpose !== 'totp-mfa' && isMfaChallengePage()) {
        return {
          success: true,
...
async function fillVerificationCode(step, payload) {
  ...
  if (splitInputs?.length >= 6) {
    ...
    if (!filled) {
      ...
      log(`步骤 ${step}：分格验证码输入框已稳定显示目标长度。`, 'info');
    }
...
const STEP5_PROFILE_SUBMIT_ACTION_PATTERN = /完成|创建|继续|下一步|提交|同意|create|continue|next|submit|finish|done|agree/i;
const STEP5_ONBOARDING_USAGE_TITLE_PATTERN = /是什么促使你使用\s*ChatGPT|what\s+(?:brings|prompted|made|motivated).{0,80}ChatGPT|what\s+will\s+you\s+use\s+ChatGPT\s+for/i;
const STEP5_ONBOARDING_READY_TITLE_PATTERN = /你已准备就绪|you(?:'|’)?re\s+(?:all\s+)?(?:set|ready)|you\s+are\s+(?:all\s+)?(?:set|ready)/i;
const STEP5_ONBOARDING_SKIP_ACTION_PATTERN = /^(?:跳过|skip)$/i;
const STEP5_ONBOARDING_NEXT_ACTION_PATTERN = /^(?:下一步|继续|next|continue)$/i;

function logStep5Action(message, level = 'info') {
  log(message, level, { step: 5, stepKey: 'fill-profile' });
}
...
async function recoverStep5AuthRetryPageIfPresent(context = '', options = {}) {
  const retryState = getStep5AuthRetryPageState();
  if (!retryState) {
    return null;
  }

  const contextLabel = context ? `${context}时` : '';
  logStep5Action(
    `步骤 5：${contextLabel}检测到认证重试页，准备点击“重试”恢复。状态：${JSON.stringify({
      url: retryState.url || location.href,
      path: retryState.path || location.pathname,
      retryEnabled: Boolean(retryState.retryEnabled),
      titleMatched: Boolean(retryState.titleMatched),
      detailMatched: Boolean(retryState.detailMatched),
      routeErrorMatched: Boolean(retryState.routeErrorMatched),
      fetchFailedMatched: Boolean(retryState.fetchFailedMatched),
      bodyTextPreview: getPageTextSnapshot().slice(0, 180),
    })}`,
    'warn'
  );

  const result = await recoverCurrentAuthRetryPage({
    flow: 'signup',
    logLabel: `步骤 5：${contextLabel}检测到认证重试页，正在点击“重试”恢复`,
    maxClickAttempts: options.maxClickAttempts ?? 3,
    pathPatterns: getStep5AuthRetryPathPatterns(),
    step: 5,
    stepKey: 'fill-profile',
    timeoutMs: options.timeoutMs ?? 15000,
    waitAfterClickMs: options.waitAfterClickMs ?? 3500,
  });

  logStep5Action(
    `步骤 5：${contextLabel}认证重试页恢复结果：${JSON.stringify({
      recovered: Boolean(result?.recovered),
      clickCount: Number(result?.clickCount) || 0,
      url: result?.url || location.href,
    })}`,
    result?.recovered ? 'ok' : 'warn'
  );

  return result || { recovered: true, clickCount: 0, url: location.href };
}

function getStep5SubmitButton() {
  const direct = document.querySelector('button[type="submit"], input[type="submit"]');
  if (isStep5SubmitButtonClickable(direct)) {
    return direct;
  }

  const candidates = document.querySelectorAll('button, [role="button"], input[type="button"], input[type="submit"]');
  return Array.from(candidates).find((el) => {
    if (!isStep5SubmitButtonClickable(el)) return false;
    const text = typeof getActionText === 'function'
      ? getActionText(el)
      : [
        el?.textContent,
        ...
        .replace(/\s+/g, ' ')
        .trim();
    const ddActionName = String(el.getAttribute?.('data-dd-action-name') || '').trim();
    return STEP5_PROFILE_SUBMIT_ACTION_PATTERN.test(text) || /^continue$/i.test(ddActionName);
  }) || null;
}
...
function getStep5ProfileForm(button = null) {
  const directForm = button?.form || button?.closest?.('form') || null;
  if (directForm) {
    return directForm;
  }

  return document.querySelector([
    'form[action*="/about-you" i]',
    'form[action*="/create-account/profile" i]',
    'form[action*="/signup/profile" i]',
    'form[action*="profile" i]',
    'form[aria-labelledby]',
    'form',
  ].join(', '));
}

function setStep5NativeInputValue(input, value) {
  if (!input) {
    return false;
  }
  ...
  try {
    ...
    if (nativeValueSetter) {
      nativeValueSetter.call(input, normalizedValue);
    } else {
      input.value = normalizedValue;
    }
  } catch {
...
function getStep5BirthdayDateValue({ year, month, day, resolvedAge, currentValue = '' } = {}) {
  ...
  if (
    ...
    return `${Math.floor(yearNumber)}-${String(Math.floor(monthNumber)).padStart(2, '0')}-${String(Math.floor(dayNumber)).padStart(2, '0')}`;
  }
  ...
  if (Number.isFinite(ageNumber) && ageNumber >= 5 && ageNumber <= 130) {
    const now = new Date();
    return `${now.getFullYear() - ageNumber}-06-15`;
  }

  return '';
}

function getStep5SafeAgeValue(resolvedAge) {
  const ageNumber = Math.floor(Number(resolvedAge));
  if (!Number.isFinite(ageNumber)) {
    return '';
  }
  if (ageNumber < 5 || ageNumber > 130) {
    return '';
  }
...
function syncStep5AboutYouFields(options = {}) {
  ...
  const form = getStep5ProfileForm(button);
  ...
  if (nameInput && String(fullName || '').trim()) {
    if (isVisibleElement(nameInput)) {
      fillInput(nameInput, String(fullName).trim());
    } else {
      setStep5NativeInputValue(nameInput, String(fullName).trim());
    }
    nameSynced = true;
  }
  ...
  if (explicitConsentInput && !String(explicitConsentInput.value || '').trim()) {
    setStep5NativeInputValue(explicitConsentInput, 'false');
  }
...
function dispatchStep5SubmitEvent(form, button) {
  if (!form) {
    return { dispatched: false, defaultPrevented: false };
  }

  let event = null;
  try {
    event = typeof SubmitEvent === 'function'
...
async function triggerStep5ProfileSubmit(button, options = {}) {
  if (!button) {
    throw new Error('步骤 5：提交资料时未找到提交按钮。URL: ' + location.href);
  }
  ...
  if (form && typeof form.requestSubmit === 'function') {
    try {
      form.requestSubmit(button);
      strategies.push('requestSubmit');
    } catch (error) {
      logStep5Action(`步骤 5：requestSubmit 提交失败，将继续尝试点击兜底。原因：${error?.message || String(error || '')}`, 'warn');
    }
    await sleep(350);
    if (!isStep5ProfileStillVisible()) {
      logStep5Action(`步骤 5：资料页提交已触发离开页面（第 ${attempt} 次，策略：${strategies.join('+')}）。`, 'info');
      return { strategies, formAction: form?.getAttribute?.('action') || '' };
    }
  }

  try {
    await clickChatGptPostProfileOnboardingButton(button);
    strategies.push('pointerClick');
  } catch (error) {
    logStep5Action(`步骤 5：指针点击提交失败，将继续尝试原生点击。原因：${error?.message || String(error || '')}`, 'warn');
  }
  await sleep(300);
  ...
  if (isStep5ProfileStillVisible()) {
    try {
      button.click?.();
      strategies.push('nativeClick');
    } catch (error) {
      logStep5Action(`步骤 5：原生点击提交失败。原因：${error?.message || String(error || '')}`, 'warn');
    }
    await sleep(300);
  }

  if (nativeSubmitFallback && form && isStep5ProfileStillVisible()) {
    const submitEventState = dispatchStep5SubmitEvent(form, button);
    strategies.push(submitEventState.defaultPrevented ? 'submitEventPrevented' : 'submitEvent');
    await sleep(150);
    if (!submitEventState.defaultPrevented && isStep5ProfileStillVisible()) {
      try {
        const nativeSubmit = Object.getOwnPropertyDescriptor(window.HTMLFormElement.prototype, 'submit')?.value;
        if (typeof nativeSubmit === 'function') {
          nativeSubmit.call(form);
          strategies.push('nativeFormSubmit');
        }
      } catch (error) {
        logStep5Action(`步骤 5：原生 form.submit 兜底失败。原因：${error?.message || String(error || '')}`, 'warn');
      }
    }
  }

  logStep5Action(
    `步骤 5：已触发资料页提交（第 ${attempt} 次，策略：${strategies.join('+') || 'none'}，兜底：${nativeSubmitFallback ? 'on' : 'off'}）。`,
    nativeSubmitFallback ? 'warn' : 'info'
  );
  return { strategies, formAction: form?.getAttribute?.('action') || '' };
}
...
function getChatGptPostProfileOnboardingPageKind() {
  const pageText = getPageTextSnapshot();
  const visibleActions = getStep5VisibleActionsSummary(12);
  const actionTexts = visibleActions.map((item) => item.text).filter(Boolean);
  if (STEP5_ONBOARDING_USAGE_TITLE_PATTERN.test(pageText)) {
    return 'usage';
  }
  if (
    actionTexts.some((text) => STEP5_ONBOARDING_SKIP_ACTION_PATTERN.test(text))
    && actionTexts.some((text) => STEP5_ONBOARDING_NEXT_ACTION_PATTERN.test(text))
    && actionTexts.some((text) => /学校|工作|个人任务|乐趣|娱乐|其他|school|work|personal|fun|other/i.test(text))
  ) {
    return 'usage';
  }
  if (STEP5_ONBOARDING_READY_TITLE_PATTERN.test(pageText)) {
    return 'ready';
  }
  let onChatGptPostProfileHost = false;
  try {
    const parsed = new URL(String(location.href || '').trim());
    const host = String(parsed.hostname || '').toLowerCase();
    const path = String(parsed.pathname || '');
    onChatGptPostProfileHost = ['chatgpt.com', 'www.chatgpt.com', 'chat.openai.com'].includes(host)
      && !/^\/(?:auth\/|create-account\/|email-verification|log-in|add-phone)(?:[/?#]|$)/i.test(path);
  } catch {
    onChatGptPostProfileHost = false;
  }
  const hasReadyContinueButton = Array.from(document.querySelectorAll('button, [role="button"], input[type="button"], input[type="submit"]'))
    .some((el) => isVisibleElement(el) && isActionEnabled(el) && /^(?:继续|continue)$/i.test(getActionText(el)));
  return onChatGptPostProfileHost && hasReadyContinueButton ? 'ready' : '';
}

function getChatGptPostProfileOnboardingButton(kindOverride = '') {
  const kind = kindOverride || getChatGptPostProfileOnboardingPageKind();
  if (!kind) {
    return null;
  }
  ...
  if (kind === 'usage') {
    const skipButton = visibleCandidates.find((el) => STEP5_ONBOARDING_SKIP_ACTION_PATTERN.test(getActionText(el)));
    if (skipButton) {
      return skipButton;
    }
    const nextButton = visibleCandidates.find((el) => STEP5_ONBOARDING_NEXT_ACTION_PATTERN.test(getActionText(el)));
    if (nextButton) {
      return nextButton;
    }
  }

  const buttonPattern = kind === 'usage'
    ? STEP5_ONBOARDING_SKIP_ACTION_PATTERN
    : /^(?:继续|continue)$/i;
  return visibleCandidates.find((el) => {
    const text = typeof getActionText === 'function'
      ? getActionText(el)
...
async function clickChatGptPostProfileOnboardingButton(button) {
  ...
  for (const [type, EventCtor] of [
    ...
    await sleep(type === 'pointerdown' || type === 'mousedown' ? 80 : 20);
  }
  button.click?.();
}

async function dismissChatGptPostProfileOnboardingIfPresent(options = {}) {
  const { logMiss = false, context = '' } = options;
  const kind = getChatGptPostProfileOnboardingPageKind();
  const button = getChatGptPostProfileOnboardingButton(kind);
  if (!kind || !button) {
    if (logMiss) {
      logStep5Action(
        `步骤 5：检查创建后引导页${context ? `（${context}）` : ''}，未找到可点击目标。诊断：${JSON.stringify({
          kind: kind || '',
          url: location.href,
          visibleActions: getStep5VisibleActionsSummary(12),
          bodyTextPreview: getPageTextSnapshot().slice(0, 220),
        })}`,
        kind ? 'warn' : 'info'
      );
    }
  ...
  const actionText = kind === 'usage' ? '跳过' : '继续';
  logStep5Action(`步骤 5：检测到 ChatGPT 创建后引导页（${kind}），准备点击“${actionText}”。页面按钮：${JSON.stringify(getStep5VisibleActionsSummary(12))}`, 'info');
  logStep5Action(`步骤 5：引导页点击目标：${JSON.stringify({ text: getActionText(button), tag: String(button.tagName || '').toLowerCase(), className: String(button.className || '').slice(0, 120), url: location.href })}`, 'info');
  await humanPause(350, 900);
  await clickChatGptPostProfileOnboardingButton(button);
  await sleep(1200);
  logStep5Action(`步骤 5：已执行“${actionText}”点击，当前 URL：${location.href}，当前引导页类型：${getChatGptPostProfileOnboardingPageKind() || '(无)'}`, 'info');
  if (kind === 'ready' && getChatGptPostProfileOnboardingPageKind() === 'ready') {
    logStep5Action('步骤 5：点击“继续”后仍停留在创建完成页，将直接跳转到 ChatGPT 首页继续后续流程。', 'warn');
    location.assign('https://chatgpt.com/');
    await sleep(300);
    logStep5Action(`步骤 5：已触发跳转到 ChatGPT 首页，当前 URL：${location.href}`, 'info');
  }
  return { handled: true, kind };
}

async function waitForDelayedStep5Onboarding(options = {}) {
  const {
    context = '',
    logCompletion = false,
    logEveryMs = 5000,
  ...
  while (Date.now() - startedAt < timeoutMs) {
    throwIfStopped();
    const result = await dismissChatGptPostProfileOnboardingIfPresent({
      logMiss: !loggedWaiting,
      context: context || '等待延迟出现的创建后引导页',
    });
    if (result?.handled) {
      logStep5Action(
        `步骤 5：延迟创建后引导页已处理完成。结果：${JSON.stringify({
          context: context || '',
          kind: result.kind || '',
    ...
    loggedWaiting = true;
    if (logEveryMs > 0 && Date.now() - lastProgressLogAt >= logEveryMs) {
      lastProgressLogAt = Date.now();
      logStep5Action(
        `步骤 5：正在等待可能延迟弹出的创建后引导页。状态：${JSON.stringify({
          context: context || '',
          elapsedMs: Date.now() - startedAt,
          timeoutMs,
          url: location.href,
          visibleActions: getStep5VisibleActionsSummary(8),
          bodyTextPreview: getPageTextSnapshot().slice(0, 180),
        })}`,
        'info'
      );
  ...
  if (logCompletion) {
    logStep5Action(
      `步骤 5：等待延迟创建后引导页超时，未发现需要点击的“跳过/继续”。状态：${JSON.stringify({
        context: context || '',
        timeoutMs,
        url: location.href,
        visibleActions: getStep5VisibleActionsSummary(8),
...
async function waitForStep5DelayedOnboardingCommand(payload = {}) {
  const timeoutMs = Math.max(1000, Number(payload.timeoutMs) || 30000);
  const pollIntervalMs = Math.max(100, Number(payload.pollIntervalMs) || 300);
  const startedAt = Date.now();
  let lastResult = null;
  let handledCount = 0;
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const result = await waitForDelayedStep5Onboarding({
      context: payload.context || '后台提交后校验',
      logCompletion: payload.logCompletion !== false && handledCount === 0,
      logEveryMs: Math.max(1000, Number(payload.logEveryMs) || 5000),
      pollIntervalMs,
  ...
  return {
    ...
    skipPostCompletionValidation: Boolean(lastResult?.kind === 'ready' || successState?.state === 'logged_in_home'),
    url: location.href,
  };
}

function getStep5VisibleActionsSummary(limit = 8) {
...
function getStep5ProfileDiagnostics() {
  const yearSpinner = document.querySelector('[role="spinbutton"][data-type="year"]');
  const monthSpinner = document.querySelector('[role="spinbutton"][data-type="month"]');
  const daySpinner = document.querySelector('[role="spinbutton"][data-type="day"]');
  ...
  const form = getStep5ProfileForm(submitButton);
  ...
  return {
    ...
    title: document.title || '',
    readyState: document.readyState || '',
    onboardingKind: getChatGptPostProfileOnboardingPageKind(),
    hasLoginProblemReturnButton: Boolean(getAuthLoginProblemReturnButton({ allowDisabled: true })),
    profileVisible: isStep5ProfileStillVisible(),
    formAction: form?.getAttribute?.('action') || '',
    formMethod: form?.getAttribute?.('method') || '',
    nameInputVisible: Boolean(nameInput && isVisibleElement(nameInput)),
    ...
    birthdaySelectsVisible: Boolean(
      yearReactSelect?.button && monthReactSelect?.button && dayReactSelect?.button
      && isVisibleElement(yearReactSelect.button)
      && isVisibleElement(monthReactSelect.button)
      && isVisibleElement(dayReactSelect.button)
    ),
    hiddenBirthdayPresent: Boolean(hiddenBirthday),
    hiddenBirthdayValuePresent: Boolean(String(hiddenBirthday?.value || '').trim()),
    submitButtonFound: Boolean(submitButton),
    submitButtonEnabled: isStep5SubmitButtonClickable(submitButton),
    submitButtonText: submitButton ? getActionText(submitButton).slice(0, 80) : '',
    visibleActions: getStep5VisibleActionsSummary(8),
    bodyTextPreview: getPageTextSnapshot().slice(0, 280),
  };
}

function logStep5ProfileDiagnostics(context, level = 'warn') {
  try {
    logStep5Action(`${context}：${JSON.stringify(getStep5ProfileDiagnostics())}`, level);
  } catch (error) {
    console.warn('[MultiPage:signup-page] failed to build step 5 diagnostics:', error?.message || error);
  }
}
...
async function waitForStep5SubmitOutcome(options = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (retryState) {
      ...
      logStep5Action(`步骤 5：资料提交后进入认证重试页，正在自动恢复（${authRetryRecoveryCount}/${maxAuthRetryRecoveries}）...`, 'warn');
      await recoverCurrentAuthRetryPage({
        flow: 'signup',
    ...
    const onboardingResult = await dismissChatGptPostProfileOnboardingIfPresent();
    if (onboardingResult?.handled) {
      if (onboardingResult.kind === 'ready') {
        return {
          state: 'post_profile_onboarding_ready_continue',
          skipPostCompletionValidation: true,
          url: location.href,
        };
      }
      lastSubmitClickAt = Date.now();
    ...
    if (
      ...
      if (isStep5SubmitButtonClickable(submitButton)) {
        submitClickCount += 1;
        logStep5Action(`步骤 5：资料提交后仍停留在资料页，正在重新点击“完成帐户创建”（第 ${submitClickCount}/${maxSubmitClicks} 次）...`, 'warn');
        await humanPause(350, 900);
        if (typeof resubmit === 'function') {
          await resubmit(submitButton, submitClickCount, {
            nativeSubmitFallback: submitClickCount >= maxSubmitClicks,
          });
        } else {
  ...
  const finalOnboardingResult = await dismissChatGptPostProfileOnboardingIfPresent();
  if (finalOnboardingResult?.handled) {
    if (finalOnboardingResult.kind === 'ready') {
      return {
        state: 'post_profile_onboarding_ready_continue',
        skipPostCompletionValidation: true,
        url: location.href,
    ...
    return await waitForStep5SubmitOutcome({
      timeoutMs: 15000,
      maxAuthRetryRecoveries,
      maxSubmitClicks: submitClickCount,
      retryClickIntervalMs,
      resubmit,
    });
  }
  ...
  if (finalStep5Error) {
    logStep5ProfileDiagnostics('步骤 5：资料提交后页面返回错误，当前诊断快照', 'warn');
    throw new Error(`步骤 5：资料提交后页面返回错误：${finalStep5Error}。URL: ${location.href}`);
  }

  logStep5ProfileDiagnostics('步骤 5：资料提交后未检测到页面跳转或恢复成功，最终诊断快照', 'warn');
  throw new Error(`步骤 5：资料提交后未检测到页面跳转或恢复成功（已点击提交 ${submitClickCount}/${maxSubmitClicks} 次）。URL: ${location.href}`);
}

async function step5_fillNameBirthday(payload) {
  const maxFlowRestarts = Math.max(1, Number(payload?.maxAuthRetryRecoveries) || 3);
  for (let attempt = 1; attempt <= maxFlowRestarts; attempt += 1) {
    const recoveryResult = await recoverStep5AuthRetryPageIfPresent(
      attempt === 1 ? '开始填写资料前' : `第 ${attempt} 次重新进入资料流程前`,
      { maxClickAttempts: 2, timeoutMs: 15000, waitAfterClickMs: 3500 }
    );
    ...
    try {
      return await step5_fillNameBirthdayOnce(payload);
    } catch (error) {
      if (/STEP5_RESTART_AFTER_AUTH_RETRY::/i.test(String(error?.message || error || ''))) {
        if (attempt >= maxFlowRestarts) {
          throw error;
        }
        logStep5Action(
          `步骤 5：认证重试页已恢复，准备重新执行资料流程（${attempt}/${maxFlowRestarts}）。`,
          'warn'
        );
        await sleep(1200);
        continue;
      ...
      const retryState = getStep5AuthRetryPageState();
      if (!retryState || attempt >= maxFlowRestarts) {
        throw error;
      }
      logStep5Action(
        `步骤 5：执行资料流程时进入认证重试页，准备恢复后重跑当前步骤（${attempt}/${maxFlowRestarts}）。上一错误：${error?.message || String(error || '')}`,
        'warn'
      );
      await recoverStep5AuthRetryPageIfPresent(
        `资料流程异常后第 ${attempt} 次`,
        { maxClickAttempts: 2, timeoutMs: 15000, waitAfterClickMs: 3500 }
      );
      await sleep(1200);
  ...
  throw new Error(`步骤 5：认证重试页已连续恢复 ${maxFlowRestarts} 次，仍无法完成资料填写。URL: ${location.href}`);
}

async function step5_fillNameBirthdayOnce(payload) {
  ...
  function completeStep5Once(extra = {}) {
    ...
    const completionPayload = {
      ...getStep5DirectCompletionPayload({
        isAgeMode: Boolean(extra.isAgeMode),
        navigationStarted: Boolean(extra.navigationStarted),
        outcome: extra.outcome || null,
      }),
      ...(extra.outcome?.skipPostCompletionValidation ? { skipPostCompletionValidation: true } : {}),
    };
  ...
  const initialSuccessState = getStep5PostSubmitSuccessState();
  if (initialSuccessState?.state === 'logged_in_home') {
    logStep5Action(`步骤 5：命令恢复时已进入 ChatGPT 首页，先短暂等待是否出现创建后引导页。URL: ${initialSuccessState.url || location.href}`, 'info');
    const delayedOnboardingResult = await waitForDelayedStep5Onboarding({
      context: 'ChatGPT 首页恢复后',
      logCompletion: true,
      logEveryMs: 5000,
      timeoutMs: 30000,
      pollIntervalMs: 300,
    });
    if (delayedOnboardingResult?.handled && delayedOnboardingResult.kind !== 'ready') {
      logStep5Action('步骤 5：ChatGPT 首页恢复后出现用途选择页，已点击跳过，继续确认后续状态。', 'info');
      const outcome = await waitForStep5SubmitOutcome({ timeoutMs: 30000 });
      return completeStep5Once({ outcome });
    }

    logStep5Action(`步骤 5：ChatGPT 首页等待后未发现必须处理的引导页，确认资料步骤完成。URL: ${location.href}`, 'ok');
    return completeStep5Once({
      outcome: {
        ...initialSuccessState,
        skipPostCompletionValidation: true,
        url: location.href,
  ...
  const preFieldRetryRecovery = await recoverStep5AuthRetryPageIfPresent('查找姓名输入框前', {
    maxClickAttempts: 2,
    timeoutMs: 15000,
    waitAfterClickMs: 3500,
  });
  if (preFieldRetryRecovery?.recovered) {
    throw new Error(`STEP5_RESTART_AFTER_AUTH_RETRY::步骤 5：查找姓名输入框前已从认证重试页恢复，需要重新执行当前资料流程。URL: ${location.href}`);
  }

  const initialOnboardingResult = await dismissChatGptPostProfileOnboardingIfPresent({
    logMiss: true,
    context: '开始填写资料前',
  });
  if (initialOnboardingResult?.handled) {
    if (initialOnboardingResult.kind === 'ready') {
      return completeStep5Once({
        outcome: {
          state: 'post_profile_onboarding_ready_continue',
          skipPostCompletionValidation: true,
    ...
    logStep5Action('步骤 5：命令恢复时已在用途选择页，已点击跳过，继续等待创建完成页或首页。', 'info');
    const outcome = await waitForStep5SubmitOutcome({ timeoutMs: 30000 });
    return completeStep5Once({ outcome });
  }

  logStep5Action(`步骤 5：正在填写姓名：${fullName}`);
  ...
  logStep5Action(`步骤 5：姓名已填写：${fullName}`);
  ...
  for (let i = 0; i < 100; i++) {
    const birthdayRetryRecovery = await recoverStep5AuthRetryPageIfPresent(`等待生日/年龄字段第 ${i + 1} 次`, {
      maxClickAttempts: 2,
      timeoutMs: 15000,
      waitAfterClickMs: 3500,
    });
    if (birthdayRetryRecovery?.recovered) {
      throw new Error(`STEP5_RESTART_AFTER_AUTH_RETRY::步骤 5：等待生日/年龄字段时已从认证重试页恢复，需要重新执行当前资料流程。URL: ${location.href}`);
    }

    const onboardingResult = await dismissChatGptPostProfileOnboardingIfPresent({
      logMiss: i === 0 || i === 20 || i === 50,
      context: `等待生日/年龄字段第 ${i + 1} 次`,
    });
    if (onboardingResult?.handled) {
      if (onboardingResult.kind === 'ready') {
        return completeStep5Once({
    ...
    if (await recoverAuthLoginProblemPageIfPresent({ step: 5, logLabel: '步骤 5：检测到资料页后的登录异常页，正在点击“返回”恢复...' })) {
      continue;
    }
    ...
    if (!loggedWaitingForBirthdayFields && i >= 20) {
      loggedWaitingForBirthdayFields = true;
      logStep5ProfileDiagnostics('步骤 5：姓名已填写，正在等待生日/年龄字段或创建后引导页出现，当前诊断快照', 'info');
    }
    if (i === 50) {
      logStep5ProfileDiagnostics('步骤 5：等待生日/年龄字段或创建后引导页超过 5 秒，当前诊断快照', 'warn');
    }
    await sleep(100);
  }
  ...
  if (birthdayMode) {
    ...
    if (yearReactSelect?.nativeSelect && monthReactSelect?.nativeSelect && dayReactSelect?.nativeSelect) {
      ...
      logStep5Action('步骤 5：检测到 React Aria 下拉生日字段，正在填写生日...');
      await humanPause(450, 1100);
      ...
      logStep5Action(`步骤 5：React Aria 生日已填写：${desiredDate}`);
    }

    if (yearSpinner && monthSpinner && daySpinner) {
      logStep5Action('步骤 5：检测到生日字段，正在填写生日...');
      ...
      logStep5Action(`步骤 5：生日已填写：${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`);
    }
  ...
  if (allConsentCheckbox) {
    if (!isStep5CheckboxChecked(allConsentCheckbox)) {
      ...
      logStep5Action('步骤 5：已勾选 “I agree to all of the following”。');
    } else {
      logStep5Action('步骤 5：“I agree to all of the following” 已勾选，跳过。');
    }
  }
  ...
  const postFieldOnboardingResult = await dismissChatGptPostProfileOnboardingIfPresent();
  if (postFieldOnboardingResult?.handled) {
    if (postFieldOnboardingResult.kind === 'ready') {
      return completeStep5Once({
        outcome: {
          state: 'post_profile_onboarding_ready_continue',
          skipPostCompletionValidation: true,
    ...
    logStep5Action('步骤 5：已在提交按钮前处理用途选择页，继续等待后续“你已准备就绪”页面...', 'info');
    const outcome = await waitForStep5SubmitOutcome({ timeoutMs: 30000 });
    return completeStep5Once({ outcome });
  }
  ...
  // Click "完成帐户创建" button
  await sleep(500);
  const preSubmitRetryRecovery = await recoverStep5AuthRetryPageIfPresent('查找提交按钮前', {
    maxClickAttempts: 2,
    timeoutMs: 15000,
    waitAfterClickMs: 3500,
  });
  if (preSubmitRetryRecovery?.recovered) {
    throw new Error(`STEP5_RESTART_AFTER_AUTH_RETRY::步骤 5：查找提交按钮前已从认证重试页恢复，需要重新执行当前资料流程。URL: ${location.href}`);
  }

  const completeBtn = await waitForStep5SubmitButton(10000);
  if (!completeBtn) {
  ...
  const initialSyncState = syncProfileFields();
  if (isAgeMode) {
    logStep5Action(
  ...
  await performOperationWithDelay({ stepKey: 'fill-profile', kind: 'submit', label: 'submit-profile' }, async () => {
    await triggerStep5ProfileSubmit(completeBtn, { attempt: 1 });
  });
  logStep5Action('步骤 5：已点击“完成帐户创建”，正在等待页面跳转、重试页或提交结果。');

  try {
    const outcome = await waitForStep5SubmitOutcome({
      resubmit: async (submitButton, attempt, retryOptions = {}) => {
        const syncState = syncStep5AboutYouFields({
          button: submitButton || completeBtn,
          fullName,
          resolvedAge,
          year,
          month,
        ...
        logStep5Action(
          `步骤 5：重新提交前已同步 about-you 表单字段：${JSON.stringify({
            attempt,
          ...
          retryOptions.nativeSubmitFallback ? 'warn' : 'info'
        );
        await triggerStep5ProfileSubmit(submitButton || completeBtn, {
          attempt,
          nativeSubmitFallback: Boolean(retryOptions.nativeSubmitFallback),
        });
      },
    ...
    logStep5Action(`步骤 5：资料提交结果已确认（${outcome.state || 'success'}），准备继续后续步骤。`, 'ok');
    return completionPayload;
  } catch (error) {
    cleanupNavigationReporter();
    throw error;
