...
function findHostedOpenAiPayPalRadio() {
  return document.querySelector('#payment-method-accordion-item-title-paypal')
    || document.querySelector('input[type="radio"][value="paypal"]');
}

function findHostedOpenAiPaymentMethodItem(method = PLUS_PAYMENT_METHOD_PAYPAL) {
  const normalized = normalizePlusPaymentMethod(method);
  return document.querySelector(`[data-testid="${normalized}-accordion-item"]`)
    || document.querySelector(`.${normalized}-accordion-item`);
}

function isHostedOpenAiPaymentMethodSelected(method = PLUS_PAYMENT_METHOD_PAYPAL) {
  const normalized = normalizePlusPaymentMethod(method);
  const radio = normalized === PLUS_PAYMENT_METHOD_PAYPAL
    ? findHostedOpenAiPayPalRadio()
    : document.querySelector(`input[type="radio"][value="${CSS.escape(normalized)}"]`);
  if (
    radio?.checked === true
  ...
  const item = findHostedOpenAiPaymentMethodItem(normalized);
  const className = typeof item?.className === 'string' ? item.className : item?.getAttribute?.('class') || '';
  return item?.getAttribute?.('aria-checked') === 'true'
    || item?.getAttribute?.('aria-selected') === 'true'
    || /\b(selected|checked|active)\b/i.test(className);
}

function getHostedOpenAiPayPalClickTargets() {
  const selectors = [
    ...
    '.paypal-accordion-item .AccordionItemHeader',
    '.paypal-accordion-item .AccordionItemCover',
  ...
  return selectors
    ...
    .filter((el) => {
      if (!el || seen.has(el) || !isVisibleElement(el) || !isEnabledControl(el)) {
        return false;
      }
      seen.add(el);
      return true;
...
function getHostedOpenAiPayPalDebuggerTargets() {
  const selectors = [
    '[data-testid="paypal-accordion-item"] .AccordionItemHeader',
    '[data-testid="paypal-accordion-item"] .AccordionItemCover',
    '[data-testid="paypal-accordion-item-button"]',
    ...
    '.paypal-accordion-item .AccordionItemHeader',
    '.paypal-accordion-item .AccordionItemCover',
    '.paypal-accordion-item .PaymentMethodFormAccordionItemTitle',
    '.paypal-accordion-item',
  ];
  const seen = new Set();
  return selectors
    .map((selector) => document.querySelector(selector))
    .filter((el) => {
      if (!el || seen.has(el) || !isVisibleElement(el)) {
        return false;
      }
      seen.add(el);
      return true;
...
function buildHostedOpenAiDebuggerClickRect(el) {
  if (!el) return null;
  ...
  if (rect.width > 160 && !/payment-method-label-paypal/i.test(String(el.id || ''))) {
    centerX = rect.left + Math.min(Math.max(rect.width * 0.22, 64), rect.width - 12);
  }
  ...
  return {
    ...
    centerX,
    centerY,
    target: summarizeElementForDebug(el),
    hit: summarizeElementForDebug(hit),
  };
}

function getHostedOpenAiPayPalDebuggerClickRects() {
  return getHostedOpenAiPayPalDebuggerTargets()
    .map(buildHostedOpenAiDebuggerClickRect)
    .filter(Boolean);
}

function getHostedOpenAiPayPalSelectionSnapshot() {
  const payPalRadio = findHostedOpenAiPayPalRadio();
  const cardRadio = document.querySelector('#payment-method-accordion-item-title-card')
    || document.querySelector('input[type="radio"][value="card"]');
  return {
    paypalButtonFound: Boolean(findHostedOpenAiPayPalButton()),
    paypalRadioFound: Boolean(payPalRadio),
    paypalChecked: Boolean(payPalRadio?.checked),
    paypalAriaChecked: payPalRadio?.getAttribute?.('aria-checked') || '',
    cardChecked: Boolean(cardRadio?.checked),
    cardAriaChecked: cardRadio?.getAttribute?.('aria-checked') || '',
    targets: getHostedOpenAiPayPalClickTargets().map(summarizeElementForDebug).filter(Boolean).slice(0, 4),
    debuggerTargets: getHostedOpenAiPayPalDebuggerClickRects().slice(0, 4),
  };
}

function buildHostedOpenAiPayPalDebuggerRequest(reason = '') {
  const rects = getHostedOpenAiPayPalDebuggerClickRects();
  return {
    selected: false,
    needsDebuggerClick: true,
    reason,
    rect: rects[0]?.centerX ? rects[0] : null,
    rects,
    snapshot: getHostedOpenAiPayPalSelectionSnapshot(),
  };
}
...
async function ensureHostedOpenAiPayPalSelected(options = {}) {
  ...
  const preferDebuggerClick = Boolean(options.preferDebuggerClick);
  const allowDebuggerRequest = options.allowDebuggerRequest !== false;
  const startedAt = Date.now();
  let targets = [];
  while (Date.now() - startedAt < timeoutMs) {
    throwIfStopped();
    if (isHostedOpenAiPaymentMethodSelected(PLUS_PAYMENT_METHOD_PAYPAL)) {
      log('Plus Checkout：hosted checkout PayPal 已处于选中状态。');
      return true;
    }
    if (preferDebuggerClick && allowDebuggerRequest) {
      const debuggerRequest = buildHostedOpenAiPayPalDebuggerRequest('preferred_debugger_click');
      if (debuggerRequest.rect) {
        return debuggerRequest;
      }
    }
    targets = getHostedOpenAiPayPalClickTargets();
    if (targets.length) {
      break;
    }
    await sleep(250);
  ...
  if (!targets.length) {
    const debuggerRequest = allowDebuggerRequest
      ? buildHostedOpenAiPayPalDebuggerRequest('paypal_targets_not_found_for_content_click')
      : null;
    if (debuggerRequest?.rect) {
      return debuggerRequest;
    }
    throw new Error(`hosted checkout 页面未找到 PayPal 付款方式。状态：${JSON.stringify(getHostedOpenAiPayPalSelectionSnapshot())}`);
  }

  for (let attempt = 1; attempt <= 3; attempt += 1) {
    throwIfStopped();
    for (const target of targets) {
      if (!isVisibleElement(target) || !isEnabledControl(target)) {
        continue;
      }
      await humanLikeClick(target);
      await sleep(700);
      if (isHostedOpenAiPaymentMethodSelected(PLUS_PAYMENT_METHOD_PAYPAL)) {
        log(`Plus Checkout：hosted checkout PayPal 已选中（第 ${attempt} 轮）。`);
        return true;
      }
    }
    targets = getHostedOpenAiPayPalClickTargets();
  ...
  const debuggerRequest = allowDebuggerRequest
    ? buildHostedOpenAiPayPalDebuggerRequest('content_click_did_not_select_paypal')
    : null;
  if (debuggerRequest?.rect) {
    return debuggerRequest;
  }
  throw new Error(`hosted checkout 已点击 PayPal，但未观察到 PayPal radio 选中；已停止提交，避免沿用银行卡。状态：${JSON.stringify(getHostedOpenAiPayPalSelectionSnapshot())}`);
}
...
async function runHostedOpenAiCheckoutStep(payload = {}) {
  ...
  const payPalSelectionResult = await ensureHostedOpenAiPayPalSelected({
    preferDebuggerClick: payload.preferDebuggerClick !== false,
    allowDebuggerRequest: payload.allowDebuggerClick !== false,
  });
  if (payPalSelectionResult?.needsDebuggerClick) {
    return {
      ...payPalSelectionResult,
...
function getCheckoutAddressErrorText() {
  const text = normalizeText(document.body?.innerText || document.body?.textContent || '');
  const patterns = [
    /The customer's location isn't recognized\.?.{0,120}?(?:automatically calculate tax|valid customer address)/i,
    /customer(?:'s)?\s+location\s+isn'?t\s+recognized/i,
    /Set\s+a\s+valid\s+customer\s+address/i,
  ];
  return patterns.find((pattern) => pattern.test(text)) ? text.slice(0, 600) : '';
}
...
async function fillPlusBillingAddress(payload = {}) {
  ...
  const overwriteStructuredAddress = Boolean(payload.overwriteStructuredAddress);
  const useDirectStructuredBranch = Boolean(seed.skipAutocomplete || overwriteStructuredAddress || isDropdownStructuredAddressForm(fields));
  ...
  const structuredAddress = await performOperationWithDelay({ stepKey: 'plus-checkout-billing', kind: 'fill', label: 'fill-billing-address' }, async () => {
    ...
    return ensureStructuredAddress(seed, {
      overwrite: useDirectStructuredBranch || overwriteStructuredAddress,
...
async function readChatGptSessionAccessToken() {
  ...
  async function warmupPaymentsSuccessPageIfNeeded() {
    try {
      const parsed = new URL(String(location.href || '').trim());
      if (!/(^|\.)chatgpt\.com$/i.test(parsed.hostname) || !/^\/payments\/success(?:[/?#]|$)/i.test(parsed.pathname)) {
        return false;
      }
  ...
  let lastResult = null;
  let warmedUpPaymentsSuccess = false;
  const maxAttempts = 4;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    lastResult = await fetchSession();
    if (lastResult.accessToken) {
      return {
        session: lastResult.session,
        accessToken: lastResult.accessToken,
        sessionReadAttempts: attempt,
        warmedUpPaymentsSuccess,
      };
    }

    if (attempt < maxAttempts) {
      if (!warmedUpPaymentsSuccess) {
        warmedUpPaymentsSuccess = await warmupPaymentsSuccessPageIfNeeded();
        if (warmedUpPaymentsSuccess) {
          log('Plus：payments/success 首次未返回 accessToken，已重新请求当前成功页，准备再次读取 session。', 'warn');
        }
      }
      await sleep(warmedUpPaymentsSuccess ? 1200 : 900);
    }
  }
  ...
  return {
    session,
    accessToken: String(session?.accessToken || session?.access_token || '').trim(),
    sessionReadAttempts: maxAttempts,
    warmedUpPaymentsSuccess,
  };
}
...
async function inspectPlusCheckoutState(options = {}) {
  ...
  const state = {
    ...
    addressErrorText: getCheckoutAddressErrorText(),
    addressFieldValues: {
  ...
  if (options.includeSession || options.includeAccessToken) {
    ...
    state.sessionReadAttempts = sessionState.sessionReadAttempts;
    state.warmedUpPaymentsSuccess = Boolean(sessionState.warmedUpPaymentsSuccess);
  }
  return state;
}
})();
