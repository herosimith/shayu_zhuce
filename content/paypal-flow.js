...
if (document.documentElement.getAttribute(PAYPAL_FLOW_LISTENER_SENTINEL) !== '1') {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (
      ...
      || message.type === 'PAYPAL_HOSTED_CLICK_RESEND_CODE'
...
async function handlePayPalCommand(message) {
  switch (message.type) {
    ...
    case 'PAYPAL_HOSTED_GET_STATE':
      return inspectPayPalState();
    case 'PAYPAL_HOSTED_CLICK_RESEND_CODE':
      return clickHostedVerificationResendCode();
...
async function humanLikeClick(el) {
  if (!el) {
    throw new Error('PayPal hosted checkout 点击目标不存在。');
  }
  ...
  if (!rect.width || !rect.height) {
    throw new Error('PayPal hosted checkout 点击目标不可见。');
  }
  ...
  for (const [type, EventCtor] of [
    ...
    await sleep(type === 'pointerdown' || type === 'mousedown' ? 70 : 20);
  }
  el.click?.();
}
...
function getHostedVerificationErrorText() {
  ...
  const match = text.match(/Sorry,\s*something\s+went\s+wrong\.\s*Get\s+a\s+new\s+code\.?|Get\s+a\s+new\s+code|出了点问题.{0,40}新验证码|重新获取验证码/i);
  return match ? match[0] : '';
}

function findHostedVerificationResendButton() {
  const direct = document.querySelector('button[data-testid="resend-link"], [data-testid="resend-link"]');
  if (direct && isVisibleElement(direct) && isEnabledControl(direct)) {
    return direct;
  }
  return getVisibleControls('button, a, [role="button"]').find((el) => (
    isEnabledControl(el) && /^(?:resend|重新发送|再次发送|获取新验证码|get\s+a\s+new\s+code)$/i.test(getActionText(el))
  )) || null;
}

async function clickHostedVerificationResendCode() {
  await waitForDocumentComplete();
  const button = findHostedVerificationResendButton();
  if (!button) {
    throw new Error('PayPal hosted checkout 未找到 Resend 验证码按钮。');
  }
  await humanLikeClick(button);
  await sleep(800);
  return {
    clicked: true,
    errorText: getHostedVerificationErrorText(),
  };
}
