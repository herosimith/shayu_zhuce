(function attachBackgroundPlusCheckoutCreate(root, factory) {
  ...
  const HOSTED_CHECKOUT_VERIFICATION_POPUP_DELAY_LEGACY_DEFAULT_SECONDS = 20;
  ...
  function createPlusCheckoutCreateExecutor(deps = {}) {
    const {
      ...
      getState = null,
      isCheckoutConversionProxyActive = null,
      requestStop = null,
      registerTab,
    ...
    function parseColonCheckoutConversionProxy(value = '') {
      ...
      const port = normalizeCheckoutConversionProxyPort(parts.shift());
    ...
    function parseCheckoutConversionProxyUrl(value = '') {
      ...
      const colonProxy = parseColonCheckoutConversionProxy(rawValue);
      if (colonProxy) {
        return colonProxy;
      }
      ...
      try {
        ...
        throw new Error('支付转换代理不是有效 URL，请填写 http://user:pass@host:port / socks5h://user:pass@host:port，或 host:port:user:pass。');
      }

      const rawProtocol = String(parsed.protocol || '').replace(/:$/g, '').trim().toLowerCase();
      const rawHost = String(parsed.hostname || '').trim();
      const rawPort = normalizeCheckoutConversionProxyPort(parsed.port);
      if ((rawProtocol === 'socks5h' || rawProtocol === 'socks5')
        && rawHost === CHECKOUT_CONVERSION_UPSTREAM_HOST
        && rawPort === CHECKOUT_CONVERSION_UPSTREAM_PORT) {
        return parseCheckoutConversionProxyUrl(CHECKOUT_CONVERSION_LOCAL_BRIDGE_URL);
      }

      const protocol = normalizeCheckoutConversionProxyProtocol(rawProtocol);
      if (!protocol) {
    ...
    function describeCheckoutConversionProxyEntry(entry = null) {
      ...
      const authLabel = entry.username ? '***@' : '';
      return `${String(entry.protocol || '').toLowerCase()}://${authLabel}${String(entry.host || '').trim()}:${Number(entry.port) || 0}`;
    }
  ...
  return "DIRECT";
    ...
    async function testCheckoutConversionProxy(options = {}) {
      ...
      try {
        ...
        let reachability = { reachable: true, skipped: true, endpoint: '', source: 'background_only' };
    ...
    async function applyCheckoutConversionProxyForScope(state = {}, options = {}) {
      ...
      const applyProxy = typeof applyCheckoutScopedProxyFromUrl === 'function'
        ? applyCheckoutScopedProxyFromUrl
        : defaultApplyCheckoutScopedProxyFromUrl;
      return applyProxy(proxyUrl, options);
    }

    async function restoreCheckoutConversionProxyForScope(snapshot = null) {
      if (!snapshot?.applied) {
        return;
      }
      const restoreProxy = typeof restoreCheckoutScopedProxySnapshot === 'function'
    ...
    async function maybeApplyCheckoutConversionProxy(state = {}, paymentMethod = PLUS_PAYMENT_METHOD_PAYPAL) {
      ...
      if (typeof isCheckoutConversionProxyActive === 'function' && isCheckoutConversionProxyActive()) {
        await addLog('步骤 6：支付转换代理已在插件执行开始时启用，继续沿用全流程代理。', 'info');
        return null;
      }
      ...
      await addLog(`步骤 6：未检测到插件全流程代理，已临时启用支付转换代理 ${displayName}。`, 'info');
      return snapshot;
    }
    ...
    async function maybeRestoreCheckoutConversionProxy(snapshot = null) {
      ...
      const restoreProxy = typeof restoreCheckoutScopedProxySnapshot === 'function'
        ...
        : defaultRestoreCheckoutScopedProxySnapshot;
      await restoreProxy(snapshot);
      await addLog('步骤 6：临时支付转换代理已释放，后续步骤恢复原网络/原代理环境。', 'info');
    }
    ...
    async function getHostedCheckoutRuntimeConfig(options = {}) {
      ...
      const rawVerificationPopupDelaySeconds = stored?.hostedCheckoutVerificationPopupDelaySeconds ?? state?.hostedCheckoutVerificationPopupDelaySeconds;
      let verificationPopupDelaySeconds = normalizeHostedCheckoutVerificationPopupDelaySeconds(rawVerificationPopupDelaySeconds);
      if (Number(rawVerificationPopupDelaySeconds) === HOSTED_CHECKOUT_VERIFICATION_POPUP_DELAY_LEGACY_DEFAULT_SECONDS) {
        verificationPopupDelaySeconds = HOSTED_CHECKOUT_VERIFICATION_POPUP_DELAY_DEFAULT_SECONDS;
        await applyHostedCheckoutRuntimePatch({
          hostedCheckoutVerificationPopupDelaySeconds: verificationPopupDelaySeconds,
        });
        await addLog(
          `步骤 6：检测到 hosted checkout 弹窗延迟仍为旧默认 ${HOSTED_CHECKOUT_VERIFICATION_POPUP_DELAY_LEGACY_DEFAULT_SECONDS} 秒，已自动调整为 ${verificationPopupDelaySeconds} 秒。`,
          'info'
        );
      }
    ...
    async function runHostedCheckoutOpenAiFlow(tabId, guestProfile) {
      ...
      let debuggerClickAttempts = 0;
      while (true) {
        initialResult = await sendTabMessageUntilStopped(tabId, PLUS_CHECKOUT_SOURCE, {
          type: 'RUN_HOSTED_OPENAI_CHECKOUT_STEP',
          source: 'background',
          payload: {
            address: guestProfile.address,
            preferDebuggerClick: debuggerClickAttempts === 0,
            allowDebuggerClick: debuggerClickAttempts < 3,
          },
        });
        if (initialResult?.error) {
          throw new Error(initialResult.error);
        }
        if (!initialResult?.needsDebuggerClick) {
          break;
        }
        if (typeof clickWithDebugger !== 'function') {
          throw new Error('步骤 6：hosted checkout 需要真实鼠标点击 PayPal，但后台 debugger 点击能力未接入。');
        }
        const rects = Array.isArray(initialResult.rects) ? initialResult.rects : [];
        const rect = rects[(debuggerClickAttempts || 0) % Math.max(1, rects.length)]
          || initialResult.rect
          || null;
        if (!rect) {
          throw new Error(`步骤 6：hosted checkout 需要真实鼠标点击 PayPal，但未返回有效坐标。状态：${JSON.stringify(initialResult.snapshot || {})}`);
        }
        debuggerClickAttempts += 1;
        await addLog(`步骤 6：PayPal 付款方式未响应普通点击，正在使用 debugger 真实鼠标事件点击 PayPal（第 ${debuggerClickAttempts}/3 次）...`, 'warn');
        await clickWithDebugger(tabId, rect, { visibleStep: 6 });
        await sleepWithStop(1200);
        if (debuggerClickAttempts >= 3) {
          continue;
        }
      }
    ...
    async function clickHostedCheckoutPayPalResendCode(tabId) {
      await ensureContentScriptReadyOnTabUntilStopped(PAYPAL_SOURCE, tabId, {
        inject: PAYPAL_INJECT_FILES,
        injectSource: PAYPAL_SOURCE,
        logMessage: '步骤 6：正在等待 PayPal hosted checkout 页面脚本就绪...',
      ...
      const result = await sendTabMessageUntilStopped(tabId, PAYPAL_SOURCE, {
        type: 'PAYPAL_HOSTED_CLICK_RESEND_CODE',
        source: 'background',
        payload: {},
      });
      if (result?.error) {
    ...
    async function runHostedCheckoutPayPalFlow(tabId, guestProfile) {
      ...
      while (Date.now() - startedAt < HOSTED_CHECKOUT_PAYPAL_LOOP_TIMEOUT_MS) {
        ...
        if (pageState.hostedStage === 'verification' && pageState.hostedVerificationErrorText && pageState.hostedVerificationResendReady) {
          await addLog('步骤 6：PayPal hosted checkout 验证码提交失败，正在点击 Resend 获取新验证码...', 'warn');
          await clickHostedCheckoutPayPalResendCode(tabId);
          await clearHostedCheckoutCurrentSmsEntry();
          await addLog('步骤 6：已点击 Resend，等待 3 秒后重新获取验证码。', 'info');
    ...
    async function executePlusCheckoutCreate(state = {}) {
      ...
      try {
        ...
        if (checkoutScopedProxySnapshot?.applied) {
          ...
          await setState({ plusCheckoutConversionProxySnapshot: null });
    ...
    return {
      applyCheckoutConversionProxyForScope,
      executePlusCheckoutCreate,
      fetchHostedCheckoutVerificationCodeManually,
      restoreCheckoutConversionProxyForScope,
