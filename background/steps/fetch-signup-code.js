(function attachBackgroundStep4(root, factory) {
  ...
  function createStep4Executor(deps = {}) {
      ...
      await resolveVerificationStep(4, state, mail, {
        ...
        onMfaChallengeRequired: async () => {
          if (typeof resolveMfaChallenge !== 'function') {
            throw new Error('步骤 4：验证码提交后进入 2FA 验证页，但 2FA 提交流程尚未初始化。');
          }
          return resolveMfaChallenge(4, state, {
            nodeId: 'fetch-signup-code',
            visibleStep: 4,
            source: 'step4_after_email_code',
          });
        },
    ...
    async function executeStep4(state) {
      ...
      if (prepareResult?.mfaChallenge) {
        if (typeof resolveMfaChallenge !== 'function') {
          throw new Error('步骤 4：检测到 2FA 验证页，但 2FA 提交流程尚未初始化。');
        }
        return resolveMfaChallenge(4, state, {
          nodeId: 'fetch-signup-code',
          visibleStep: 4,
          source: 'step4_prepare',
        });
      }
