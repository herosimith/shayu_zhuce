(function attachBackgroundStep1(root, factory) {
  ...
  function createStep1Executor(deps = {}) {
    ...
    function shouldSkipStep1CookieCleanup(state = {}) {
      return Boolean(
        state?.skipOpenChatgptCookieCleanupOnce
        || state?.preserveOpenChatgptCookiesOnce
      );
    }

    async function clearOpenAiCookiesBeforeStep1(state = {}) {
      if (shouldSkipStep1CookieCleanup(state)) {
        await addLog('步骤 1：本次为轮询/续跑重开，已跳过 ChatGPT / OpenAI cookies 清理，避免 accessToken 失效。', 'info');
    ...
    async function executeStep1(state = {}) {
      await clearOpenAiCookiesBeforeStep1(state);
