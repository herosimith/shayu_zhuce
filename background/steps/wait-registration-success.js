(function attachBackgroundStep6(root, factory) {
  ...
  function createStep6Executor(deps = {}) {
    ...
    async function clearCookiesIfEnabled(state = {}) {
      ...
      const isAutoRunFlow = Boolean(state?.autoRunning)
        || ['running', 'waiting_step', 'retrying', 'waiting_interval'].includes(phase)
        || Number(state?.autoRunSessionId || 0) > 0;
      if (isAutoRunFlow) {
        await addLog('步骤 6：自动/轮询流程中已跳过 Cookies 清理，避免 accessToken 失效。', 'info');
        return;
      }
