  ...
  function createSub2ApiSessionImportExecutor(deps = {}) {
    ...
    function isPaymentsSuccessUrl(url = '') {
      try {
        ...
        const hostname = String(parsed.hostname || '').trim().toLowerCase();
        return /(^|\.)chatgpt\.com$/.test(hostname)
          && /^\/payments\/success(?:[/?#]|$)/i.test(String(parsed.pathname || ''));
      } catch {
        return false;
      }
    }
    ...
    async function requestChatGptSessionState(tabId, visibleStep) {
    ...
    async function refreshSessionTabForAccessToken(tabId, visibleStep, reason = '') {
      const tab = await chrome?.tabs?.get?.(tabId).catch(() => null);
      const currentUrl = String(tab?.url || '').trim();
      if (!chrome?.tabs?.reload || !isPaymentsSuccessUrl(currentUrl)) {
        return false;
      }

      await addStepLog(
        visibleStep,
        `ChatGPT 支付成功页首次未返回 accessToken，正在刷新当前 success 页面后重读 session${reason ? `（${reason}）` : ''}...`,
        'warn'
      );
    ...
    async function readCurrentChatGptSession(tabId, visibleStep) {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        lastSessionState = await requestChatGptSessionState(tabId, visibleStep);
        if (lastSessionState.accessToken) {
          if (attempt > 1) {
            await addStepLog(visibleStep, `已在第 ${attempt} 次读取到 ChatGPT accessToken，继续导入 SUB2API。`, 'ok');
          }
          return {
            session: lastSessionState.session,
            accessToken: lastSessionState.accessToken,
          };
        }

        const hasSessionObject = Boolean(lastSessionState.session);
        const detail = `第 ${attempt}/3 次读取结果：session=${hasSessionObject ? '有' : '无'}，accessToken=空`;
        if (attempt === 1 && await refreshSessionTabForAccessToken(tabId, visibleStep, detail)) {
          continue;
        }
        if (attempt < 3) {
          await addStepLog(visibleStep, `${detail}，等待后再次读取...`, 'warn');
          await sleepWithStop(1500);
        }
      }

      throw new Error(`步骤 ${visibleStep}：未读取到有效的 ChatGPT accessToken，请确认当前 payments/success 页面已完成加载且账号仍处于已登录状态。`);
    }
