  ...
  function createStep8Executor(deps = {}) {
    ...
    async function recoverStep8PollingFailure(currentState, visibleStep) {
      ...
      if (pageState?.state === 'oauth_consent_page') {
        ...
        if (pageState?.state === 'mfa_challenge_page') {
          if (typeof resolveMfaChallenge !== 'function') {
            throw new Error(`步骤 ${visibleStep}：检测到 2FA 验证页，但 2FA 提交流程尚未初始化。`);
          }
          await resolveMfaChallenge(8, currentState, {
            nodeId: currentState?.nodeId || 'fetch-login-code',
            visibleStep,
    ...
    async function pollEmailVerificationCode(preparedState, pageState, visibleStep, runtime = {}) {
      ...
      await resolveVerificationStep(8, {
        ...
        onMfaChallengeRequired: async () => {
          if (typeof resolveMfaChallenge !== 'function') {
            throw new Error(`步骤 ${visibleStep}：验证码提交后进入 2FA 验证页，但 2FA 提交流程尚未初始化。`);
          }
          return resolveMfaChallenge(8, {
            ...preparedState,
            step8VerificationTargetEmail: displayedVerificationEmail || '',
          }, {
            nodeId: preparedState?.nodeId || (visibleStep >= 11 ? 'fetch-bound-email-login-code' : 'fetch-login-code'),
            visibleStep,
            source: 'step8_after_email_code',
          });
        },
    ...
    async function executeBoundEmailLoginCode(state) {
      ...
      if (pageState?.state === 'mfa_challenge_page') {
        ...
        return resolveMfaChallenge(8, preparedState, {
          nodeId: state?.nodeId || 'fetch-bound-email-login-code',
          visibleStep,
          source: 'bound_email_login',
        });
      }
      if (pageState?.state === 'add_phone_page' || pageState?.state === 'phone_verification_page') {
    ...
    async function runStep8Attempt(state, runtime = {}) {
      ...
      if (pageState?.state === 'mfa_challenge_page') {
        ...
        return resolveMfaChallenge(8, state, {
          nodeId: state?.nodeId || 'fetch-login-code',
          visibleStep,
          source: 'step8_ready',
        });
      }
