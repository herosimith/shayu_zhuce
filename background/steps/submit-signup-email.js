(function attachBackgroundStep2(root, factory) {
  ...
  function createStep2Executor(deps = {}) {
    ...
    async function failStep2OnLoggedInSession(tabId, reasonMessage = '') {
      ...
      const message = `SIGNUP_USER_ALREADY_EXISTS::步骤 2：检测到当前停留在已登录 ChatGPT 首页，当前邮箱可能已注册或当前会话已登录；已阻止自动跳过步骤 3/4/5，并准备换下一个未用邮箱。${reasonSuffix}`;
    ...
    async function executeSignupEmailEntry(state) {
      ...
      const step2AlreadyPastIdentityPage = Boolean(
        step2Result?.alreadyOnPasswordPage
        || step2Result?.alreadyOnVerificationPage
        || step2Result?.alreadyOnPhoneVerificationPage
        || step2Result?.alreadyOnProfilePage
        || step2Result?.alreadyLoggedInHome
      );

      if (!step2AlreadyPastIdentityPage) {
        await addLog(`步骤 2：邮箱 ${resolvedEmail} 已提交，正在等待页面加载并确认下一步入口...`);
      }

      let landingResult = await ensureSignupPostEmailPageReadyInTab(signupTabId, 2, {
        skipUrlWait: step2AlreadyPastIdentityPage,
      });

      if (landingResult?.postIdentityTimeout && landingResult?.state === 'email_entry') {
        await addLog('步骤 2：邮箱提交后页面仍停留在输入页，正在重新提交一次邮箱...', 'warn');
        step2Result = await submitSignupEmail(resolvedEmail, {
          timeoutMs: 45000,
          retryDelayMs: 700,
          logMessage: '步骤 2：页面仍在邮箱输入页，正在重新提交邮箱...',
        });
        if (step2Result?.error) {
      ...
      if (landingResult?.postIdentityTimeout && landingResult?.state === 'entry_home') {
        throw new Error(`步骤 2：提交邮箱后页面回到注册入口页，可能“继续”点击未生效或页面拒绝该邮箱。URL: ${landingResult.url || 'unknown'}`);
      }

      if (landingResult?.state === 'logged_in_home' || step2Result?.alreadyLoggedInHome) {
        const homeUrl = landingResult?.url || step2Result?.url || '';
        throw new Error(
          `SIGNUP_USER_ALREADY_EXISTS::步骤 2：邮箱 ${resolvedEmail} 提交后直接进入 ChatGPT 首页，通常表示该邮箱已注册或当前会话已登录；已将当前邮箱视为不可继续注册。URL: ${homeUrl || 'unknown'}`
        );
      }
      ...
      await completeNodeFromBackground('submit-signup-email', {
        ...
        nextSignupState: landingResult?.state || step2Result?.state || 'password_page',
        nextSignupUrl: landingResult?.url || step2Result?.url || '',
        skippedPasswordStep: landingResult?.state === 'verification_page' || step2Result?.alreadyOnVerificationPage,
        skipProfileStep: Boolean(step2Result?.skipProfileStep),
      });
    }
