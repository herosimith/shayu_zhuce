(function attachBackgroundPlusCheckoutBilling(root, factory) {
  ...
  function createPlusCheckoutBillingExecutor(deps = {}) {
    ...
    function isSameAddressSeed(left = null, right = null) {
      const leftFallback = left?.fallback || {};
      const rightFallback = right?.fallback || {};
      const normalize = (value) => normalizeText(value).toLowerCase();
      return Boolean(
        normalize(leftFallback.address1)
        && normalize(leftFallback.address1) === normalize(rightFallback.address1)
        && normalize(leftFallback.city) === normalize(rightFallback.city)
        && normalize(leftFallback.postalCode) === normalize(rightFallback.postalCode)
      );
    }
    ...
    async function resolveBillingAddressSeed(state = {}, countryOverride = '', options = {}) {
      ...
      const avoidAddressSeed = options?.avoidAddressSeed || null;
      ...
      if (paymentMethod === PLUS_PAYMENT_METHOD_PAYPAL && countryCode === 'US') {
        for (let randomAttempt = 1; randomAttempt <= 3; randomAttempt += 1) {
          try {
            const randomUserSeed = await fetchRandomUserUsAddressSeed(localSeed);
            if (hasCompleteAddressFallback(randomUserSeed) && !isSameAddressSeed(randomUserSeed, avoidAddressSeed)) {
              await addLog(
            ...
            if (isSameAddressSeed(randomUserSeed, avoidAddressSeed)) {
              await addLog(`步骤 7：randomuser.me 返回了与上次相同的美国地址，正在重新获取（${randomAttempt}/3）...`, 'warn');
              continue;
            }
    ...
    async function executePlusCheckoutBilling(state = {}) {
      ...
      async function fillBillingAddressWithSeed(addressSeed, options = {}) {
        ...
        if (!addressSeed.skipAutocomplete && autocompleteFrame?.frame && autocompleteFrame.frame.frameId !== billingFrame.frameId) {
          ...
          const structuredResult = await sendFrameMessage(tabId, billingFrame.frameId, {
            ...
            payload: {
              addressSeed,
              overwriteStructuredAddress: Boolean(suggestionError || options.overwrite),
              autoCheckAgreement: Boolean(addressSeed.autoCheckAgreement),
            },
          });
        ...
        const directResult = await sendFrameMessage(tabId, billingFrame.frameId, {
          ...
          payload: {
            ...
            addressSeed,
            autoCheckAgreement: Boolean(addressSeed.autoCheckAgreement),
            overwriteStructuredAddress: Boolean(options.overwrite),
          },
        });
      ...
      let addressSeed = await resolveBillingAddressSeed(billingState, billingFrame.countryText, { paymentMethod });
      let result = await fillBillingAddressWithSeed(addressSeed);
      ...
      for (let attempt = 1; attempt <= PLUS_CHECKOUT_SUBMIT_MAX_ATTEMPTS; attempt += 1) {
        ...
        const postSubmitState = await sendFrameMessage(tabId, billingFrame.frameId, {
          type: 'PLUS_CHECKOUT_GET_STATE',
          source: 'background',
          payload: {},
        }).catch(() => null);
        if (postSubmitState?.addressErrorText) {
          lastSubmitError = 'Stripe/OpenAI 未识别当前账单地址';
          await addLog(`步骤 7：检测到账单地址无效提示，正在重新生成美国地址并覆盖重填（${attempt}/${PLUS_CHECKOUT_SUBMIT_MAX_ATTEMPTS}）...`, 'warn');
          const nextAddressSeed = await resolveBillingAddressSeed(billingState, billingFrame.countryText, {
            paymentMethod,
            avoidAddressSeed: addressSeed,
          });
          addressSeed = nextAddressSeed;
          result = await fillBillingAddressWithSeed(addressSeed, { overwrite: true });
          await setState({
            plusCheckoutTabId: tabId,
            plusBillingCountryText: result?.countryText || '',
            plusBillingAddress: result?.structuredAddress || null,
