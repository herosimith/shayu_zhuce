(function attachBackgroundMessageRouter(root, factory) {
  ...
  function createMessageRouter(deps = {}) {
    const {
      ...
      executeWithCheckoutConversionProxy = null,
      applyPluginProxy = null,
      ...
      testFeishuSyncConnection = null,
      syncCurrentFeishuAccountResult = null,
      fetchGeneratedEmail,
      ...
      normalizeRunCount,
      ensureMultiThreadLocalServices = null,
      prepareMultiThreadWorkbench = null,
    ...
    function normalizeCustomEmailPoolEntriesForClear(payloadEntries = [], currentState = {}) {
      ...
      const legacyPool = Array.isArray(currentState?.customEmailPool) ? currentState.customEmailPool : [];
      ...
      return entries.map((entry, index) => {
        ...
        const email = String(rawEntry.email || '').trim().toLowerCase();
    ...
    async function lockAutomationWindowFromMessage(message = {}, sender = {}) {
      let windowId = resolveAutomationWindowIdFromMessage(message, sender);
      if (windowId === null && chrome?.windows?.getCurrent) {
        const currentWindow = await chrome.windows.getCurrent().catch(() => null);
        windowId = normalizeAutomationWindowId(currentWindow?.id);
      }
      if (windowId === null) {
        if (typeof setState === 'function') {
          await setState({ automationWindowId: null });
        }
        return null;
      }
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'RESOLVE_PLUS_MANUAL_CONFIRMATION': {
          ...
          if (confirmed) {
            await addLog(`步骤 ${step}：已确认手动操作完成，准备继续下一步。`, 'ok');
        ...
        case 'READ_CHATGPT_ACCESS_TOKEN': {
          if (typeof readChatGptAccessTokenInfo !== 'function') {
        ...
        case 'TEST_FEISHU_SYNC': {
          if (typeof testFeishuSyncConnection !== 'function') {
            throw new Error('飞书同步能力未接入。');
          }
          return await testFeishuSyncConnection();
        }

        case 'SYNC_FEISHU_NOW': {
          if (typeof syncCurrentFeishuAccountResult !== 'function') {
            throw new Error('飞书手动同步能力未接入。');
          }
          return await syncCurrentFeishuAccountResult(message.payload || {});
        }

        case 'POLL_EXTERNAL_REDEEM_QUEUE': {
          if (typeof pollExternalRedeemQueue !== 'function') {
            throw new Error('外部兑换队列能力未接入。');
          }
          const result = await pollExternalRedeemQueue('manual');
          return { ok: true, ...result, state: await getState() };
        }

        case 'RETRY_EXTERNAL_REDEEM_ITEM': {
          if (typeof retryExternalRedeemQueueItem !== 'function') {
            throw new Error('外部兑换重试能力未接入。');
          }
          return await retryExternalRedeemQueueItem(message.payload?.itemId || '');
        }

        case 'DELETE_EXTERNAL_REDEEM_ITEM': {
          if (typeof deleteExternalRedeemQueueItem !== 'function') {
            throw new Error('外部兑换删除能力未接入。');
          }
          return await deleteExternalRedeemQueueItem(message.payload?.itemId || '');
        }

        case 'GET_EXTERNAL_REDEEM_RECORDS': {
          if (typeof readExternalRedeemRecordsFromSqlite !== 'function') {
            throw new Error('外部兑换 SQLite 记录能力未接入。');
          }
          const result = await readExternalRedeemRecordsFromSqlite(message.payload || {});
          return { ...result, state: await getState() };
        }

        case 'CLEAR_EXTERNAL_REDEEM_RECORDS': {
          if (typeof clearExternalRedeemRecordsFromSqlite !== 'function') {
            throw new Error('外部兑换历史清空能力未接入。');
          }
          return await clearExternalRedeemRecordsFromSqlite(message.payload || {});
        }

        case 'CLEAR_EXTERNAL_REDEEM_CDKEY_HISTORY': {
          const currentState = await getState();
          const currentQueue = Array.isArray(currentState?.externalRedeemQueue)
            ? currentState.externalRedeemQueue.filter((item) => item && typeof item === 'object')
            : [];
          const shouldRemove = (item = {}) => {
            const cdkey = typeof normalizeExternalRedeemCdkey === 'function'
              ? normalizeExternalRedeemCdkey(item?.cdkey)
              : String(item?.cdkey || '').trim();
            return Boolean(cdkey);
          };
          const nextQueue = currentQueue.filter((item) => !shouldRemove(item));
          const updates = {
          ...
          if (typeof ensureExternalRedeemMonitorAlarm === 'function') {
            await ensureExternalRedeemMonitorAlarm({ ...currentState, ...updates });
          }
          ...
          if (typeof clearExternalRedeemRecordsFromSqlite === 'function') {
            try {
              const recordResult = await clearExternalRedeemRecordsFromSqlite({ silent: true });
              recordsCleared = recordResult?.ok !== false;
              recordsError = recordsCleared ? '' : String(recordResult?.error || '').trim();
            } catch (error) {
              recordsError = String(error?.message || error || '').trim();
            }
          }
          await addLog(
            recordsCleared
              ? '外部兑换：已清空 CDK 使用历史，CDK 池保留，可重新提交二次兑换。'
              : `外部兑换：已清空本地队列 CDK 使用历史，CDK 池保留；SQLite 记录清理${recordsError ? `失败：${recordsError}` : '未执行'}。`,
            recordsCleared ? 'ok' : 'warn'
          );
          return {
            ok: true,
            removedQueue: currentQueue.length - nextQueue.length,
            recordsCleared,
            recordsError,
        ...
        case 'CLEAR_EXTERNAL_REDEEM_CDKEY_POOL': {
          const currentState = await getState();
          const queue = Array.isArray(currentState?.externalRedeemQueue)
            ? currentState.externalRedeemQueue.filter((item) => {
              const status = String(item?.status || '').trim().toLowerCase();
              const hasTask = String(item?.taskId || item?.task_id || '').trim();
              if (item?.accepted === true || hasTask) {
                return true;
              }
              return !['submit_failed', 'rejected', 'not_found'].includes(status);
            })
            : [];
          const updates = {
            externalRedeemCdkeyPoolText: '',
            externalRedeemQueue: queue,
            externalRedeemLastError: '',
            externalRedeemLastSyncAt: Date.now(),
          };
          await setPersistentSettings({ externalRedeemCdkeyPoolText: '' });
          await setState(updates);
          if (typeof broadcastDataUpdate === 'function') {
            broadcastDataUpdate(updates);
          }
          ...
          await addLog('外部兑换：已清理 CDK 池，并移除本地提交失败的兑换记录。', 'ok');
          return {
            ok: true,
            state: await getState(),
          };
        ...
        case 'EXECUTE_NODE': {
          ...
          const executeManualChain = async () => {
          ...
          if (message.source === 'sidepanel' && typeof executeWithCheckoutConversionProxy === 'function') {
            await executeWithCheckoutConversionProxy(executeManualChain, {
              startLog: '支付转换代理已启用：本次插件手动执行期间，全流程网络都会走该代理。',
              finishReason: '手动节点执行结束',
            });
          } else {
            await executeManualChain();
          }
          return { ok: true };
        }
        ...
        case 'PREPARE_MULTI_THREAD_WORKBENCH': {
          if (typeof prepareMultiThreadWorkbench !== 'function') {
            throw new Error('多线程工作台能力未接入。');
          }
          return await prepareMultiThreadWorkbench(message.payload || {});
        }

        case 'ENSURE_LOCAL_SERVICES': {
          if (typeof ensureMultiThreadLocalServices !== 'function') {
            throw new Error('本地服务启动能力未接入。');
          }
          const result = await ensureMultiThreadLocalServices(message.payload || {});
          if (result?.ok === false) {
            return {
              ok: false,
              message: result.message || result.error || '本地服务启动失败',
              state: result.state || await getState(),
            };
          }
          return {
            ok: true,
        ...
        case 'SYNC_MULTI_THREAD_RUNNER_LOGS': {
          if (typeof syncMultiThreadRunnerLogs !== 'function') {
            throw new Error('多线程日志同步能力未接入。');
          }
          return await syncMultiThreadRunnerLogs(message.payload || {});
        }

        case 'START_MULTI_THREAD_AUTO_RUN': {
          if (typeof startMultiThreadAutoRun !== 'function') {
            throw new Error('多线程自动运行能力未接入。');
          }
          if (message.source === 'sidepanel') {
            await lockAutomationWindowFromMessage(message, sender);
          }
          return await startMultiThreadAutoRun(message.payload || {});
        }

        case 'STOP_MULTI_THREAD_AUTO_RUN': {
          if (typeof stopMultiThreadAutoRun !== 'function') {
        ...
        case 'SAVE_SETTING': {
          ...
          if (
            Object.prototype.hasOwnProperty.call(updates, 'customEmailPoolEntries')
            && Array.isArray(updates.customEmailPoolEntries)
            && updates.customEmailPoolEntries.length === 0
          ) {
            const existingEntries = Array.isArray(currentState?.customEmailPoolEntries)
              ? currentState.customEmailPoolEntries
              : [];
            const existingPool = Array.isArray(currentState?.customEmailPool)
              ? currentState.customEmailPool
              : [];
            if (existingEntries.length > 0 || existingPool.length > 0) {
              updates.customEmailPoolEntries = existingEntries;
              updates.customEmailPool = existingPool.length > 0
                ? existingPool
                : existingEntries.filter((entry) => entry?.enabled !== false).map((entry) => entry.email).filter(Boolean);
            }
          }
          ...
          const plusPaymentChanged = Object.prototype.hasOwnProperty.call(updates, 'plusPaymentMethod')
            && normalizePlusPaymentMethodForDisplay(currentState?.plusPaymentMethod || 'checkout-conversion')
              !== normalizePlusPaymentMethodForDisplay(updates.plusPaymentMethod || 'checkout-conversion');
          ...
          if (
            Object.prototype.hasOwnProperty.call(updates, 'externalRedeemEnabled')
            || Object.prototype.hasOwnProperty.call(updates, 'externalRedeemPollSeconds')
            || Object.prototype.hasOwnProperty.call(updates, 'externalRedeemApiKey')
            || Object.prototype.hasOwnProperty.call(updates, 'externalRedeemBaseUrl')
            || Object.prototype.hasOwnProperty.call(updates, 'externalRedeemCdkeyPoolText')
          ) {
            if (typeof ensureExternalRedeemMonitorAlarm === 'function') {
              await ensureExternalRedeemMonitorAlarm(mergedState);
            }
          }
          ...
          if (modeChanged) {
            const selectedPlusPaymentMethod = getPlusPaymentMethodLabel(
              stateUpdates.plusPaymentMethod ?? currentState?.plusPaymentMethod ?? 'checkout-conversion'
            );
            await addLog(
              Boolean(updates.plusModeEnabled)
        ...
        case 'CLEAR_CUSTOM_EMAIL_POOL_USED_FLAGS': {
          const currentState = await getState();
        ...
        case 'CLEAR_CUSTOM_EMAIL_POOL_ALL': {
          const updates = {
          ...
          await addLog('iCloud API 邮箱池：已清空全部邮箱。', 'ok');
          return {
            ok: true,
            state: await getState(),
          };
        ...
        case 'SAVE_PLUS_CHECKOUT_CONVERSION_PROXY_POOL_RESULTS': {
          if (isAutoRunLockedState(await getState())) {
            throw new Error('自动流程运行中，当前不能保存代理池测试结果。');
          }
          const results = Array.isArray(message?.payload?.results)
            ? message.payload.results.map((item, index) => ({
              index: Number.isInteger(Number(item?.index)) ? Math.max(0, Math.floor(Number(item.index))) : index,
              ok: Boolean(item?.ok),
              exitIp: String(item?.exitIp || '').trim(),
              exitRegion: String(item?.exitRegion || '').trim(),
              error: String(item?.error || '').trim().slice(0, 500),
            }))
            : [];
          await setState({ plusCheckoutConversionProxyPoolResults: results });
          return { ok: true, results };
        }

        case 'APPLY_PLUGIN_PROXY': {
          if (isAutoRunLockedState(await getState())) {
          ...
          const proxyUrl = typeof normalizeCheckoutConversionProxyInput === 'function'
            ...
            : String(message?.payload?.proxyUrl || '').trim();
          if (proxyUrl && typeof setPersistentSettings === 'function') {
            await setPersistentSettings({ plusCheckoutConversionProxyUrl: proxyUrl });
          }
          const result = await applyPluginProxy(message.payload || {});
          return {
            ok: true,
        ...
        case 'CLEAR_PLUGIN_PROXY': {
          if (isAutoRunLockedState(await getState())) {
            throw new Error('自动流程运行中，当前不能手动清除代理。');
          }
          const result = await clearPluginProxy('用户手动清除');
          return {
            ok: true,
