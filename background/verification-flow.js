(function attachBackgroundVerificationFlow(root, factory) {
  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    async function detectStep4AlreadyAdvancedDuringPolling(options = {}) {
      const signupTabId = Number(options.tabId || await getTabId('signup-page'));
      if (!signupTabId) {
        return {
          success: false,
          reason: 'missing_tab',
      ...
      try {
        ...
        // The tab may be navigating or already replaced; the mail polling path can continue.
      }
    ...
    async function buildStep4AlreadyAdvancedVerificationResult(options = {}) {
      const fallback = await detectStep4AlreadyAdvancedDuringPolling(options);
      if (!fallback.success) return null;

      const fallbackLabel = fallback.reason === 'chatgpt_home'
        ? 'ChatGPT 首页'
        : '注册资料页';
      await addLog(`步骤 4：检测到认证页已进入${fallbackLabel}，停止邮箱轮询并按验证码步骤完成。`, 'warn');
      return {
        alreadyVerified: true,
        emailTimestamp: Date.now(),
        targetEmail: '',
        source: 'signup_page_state',
        skipProfileStep: Boolean(fallback.skipProfileStep),
        skipProfileStepReason: fallback.reason === 'chatgpt_home'
          ? 'already_logged_in_home'
          : 'already_on_signup_profile',
        url: fallback.url || '',
      };
    }
    ...
    async function pollFreshVerificationCode(step, state, mail, pollOverrides = {}) {
      ...
      if (mail.provider === ICLOUD_API_PROVIDER) {
        if (typeof pollIcloudApiVerificationCode !== 'function') {
          throw new Error('iCloud API 轮询模块未加载。');
        }
        const timedPoll = await applyMailPollingTimeBudget(step, {
          ...getVerificationPollPayload(step, state),
          ...cleanPollOverrides,
        }, cleanPollOverrides, `轮询${getVerificationCodeLabel(step)}验证码邮箱`);
        return pollIcloudApiVerificationCode(step, state, {
          ...timedPoll.payload,
          ...(step === 4
            ? { alreadyAdvancedCheck: () => buildStep4AlreadyAdvancedVerificationResult() }
            : {}),
        });
      }
    ...
    async function resolveVerificationStep(step, state, mail, options = {}) {
      ...
      if (mail.provider === HOTMAIL_PROVIDER) {
        ...
        for (let attempt = 1; attempt <= maxSubmitAttempts; attempt++) {
          ...
          if (step === 4) {
            const alreadyAdvanced = await buildStep4AlreadyAdvancedVerificationResult();
            if (alreadyAdvanced) {
              const completionNodeId = await getNodeIdForStep(completionStep);
              if (!completionNodeId) {
                throw new Error(`步骤 ${completionStep} 未映射到验证码节点。`);
              ...
              await setState({
                lastEmailTimestamp: alreadyAdvanced.emailTimestamp,
                signupVerificationRequestedAt: null,
              });
              await completeNodeFromBackground(completionNodeId, {
                emailTimestamp: alreadyAdvanced.emailTimestamp,
                code: '',
                phoneVerificationRequired: false,
                skipProfileStep: Boolean(alreadyAdvanced.skipProfileStep),
                ...(alreadyAdvanced.skipProfileStepReason
                  ? { skipProfileStepReason: alreadyAdvanced.skipProfileStepReason }
                  : {}),
              });
          ...
          if (result?.alreadyVerified) {
            ...
            await setState({
              lastEmailTimestamp: result.emailTimestamp || Date.now(),
              signupVerificationRequestedAt: null,
            });
            await completeNodeFromBackground(completionNodeId, {
              ...
              code: '',
              phoneVerificationRequired: false,
              ...(step === 4 && result?.skipProfileStep ? { skipProfileStep: true } : {}),
              ...(step === 4 && result?.skipProfileStepReason
                ? { skipProfileStepReason: result.skipProfileStepReason }
                : {}),
            });
            triggerPostSuccessMailboxCleanup(step, mail);
            return {
          ...
          await addLog(`步骤 ${step}：已获取${getVerificationCodeLabel(step)}验证码。`);
          if (beforeSubmit) {
          ...
          if (submitResult.invalidCode) {
            ...
            await addLog(`步骤 ${step}：验证码被页面拒绝：${submitResult.errorText || '页面未给出具体原因'}`, 'warn');
          ...
          if (submitResult?.mfaChallengeRequired) {
            triggerPostSuccessMailboxCleanup(step, mail);
            if (typeof options.onMfaChallengeRequired === 'function') {
              return options.onMfaChallengeRequired(submitResult, {
                step,
                completionStep,
                codeResult: result,
                mail,
            ...
            throw new Error(`步骤 ${completionStep}：验证码提交后进入 2FA 验证页，但当前流程未配置 2FA 自动提交处理。`);
          }
