(function attachBackgroundAutoRunController(root, factory) {
  ...
  function createAutoRunController(deps = {}) {
    ...
    function resolveCurrentEmailFromState(state = {}) {
      const accountIdentifierType = String(state?.accountIdentifierType || '').trim().toLowerCase();
      const accountIdentifier = accountIdentifierType === 'email'
    ...
    function isRecoverableBadEmailCredentialFailure(error) {
      return typeof isIcloudApiAuthFailureError === 'function'
        && isIcloudApiAuthFailureError(error);
    }
        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          if (!useExistingProgress) {
            ...
            const keepSettings = {
              ...
              chatgptAccessTokenRecords: prevState.chatgptAccessTokenRecords,
              chatgptAccessTokenHistory: prevState.chatgptAccessTokenHistory,
              plusCheckoutConversionProxyPoolResults: prevState.plusCheckoutConversionProxyPoolResults,
              externalRedeemQueue: prevState.externalRedeemQueue,
              externalRedeemLastSyncAt: prevState.externalRedeemLastSyncAt,
              externalRedeemLastError: prevState.externalRedeemLastError,
              externalRedeemRecords: prevState.externalRedeemRecords,
              externalRedeemRecordsDbPath: prevState.externalRedeemRecordsDbPath,
              externalRedeemRecordsLastSyncAt: prevState.externalRedeemRecordsLastSyncAt,
              externalRedeemRecordsLastError: prevState.externalRedeemRecordsLastError,
              autoRunRoundSummaries: serializeAutoRunRoundSummaries(totalRuns, roundSummaries),
              autoRunSessionId: sessionId,
              tabRegistry: {},
              sourceLastUrls: {},
          ...
          try {
            ...
            const blockedByExternalRedeemQualifiedFailure = typeof isExternalRedeemQualifiedFailureError === 'function'
              && isExternalRedeemQualifiedFailureError(err);
            let reason = getErrorMessage(err);
            if (blockedByExternalRedeemQualifiedFailure && typeof getExternalRedeemQualifiedFailureMessage === 'function') {
              reason = getExternalRedeemQualifiedFailureMessage(err);
            }
            ...
            const blockedByBadEmailCredential = isRecoverableBadEmailCredentialFailure(err);
            ...
            if (blockedByExternalRedeemQualifiedFailure) {
              cancelPendingCommands('当前轮因 AC 或外部兑换未成功已终止。');
              await broadcastStopToContentScripts();

              let hasNextUnusedCustomEmail = false;
              if (typeof hasUnusedCustomEmailPoolEntry === 'function') {
                try {
                  hasNextUnusedCustomEmail = hasUnusedCustomEmailPoolEntry(await getState());
                } catch (_err) {
                  hasNextUnusedCustomEmail = false;
                }
              }

              if (hasNextUnusedCustomEmail) {
                ...
                await addLog(`第 ${targetRun}/${totalRuns} 轮 AC 或外部兑换未成功，当前邮箱已标记不可再用。原因：${reason}`, 'warn');
                await addLog(`第 ${targetRun}/${totalRuns} 轮不计为失败，将立即换下一个未用邮箱重新执行当前目标。`, 'warn');
              ...
              await addLog(`第 ${targetRun}/${totalRuns} 轮 AC 或外部兑换未成功，当前邮箱已标记不可再用。原因：${reason}`, 'warn');
              await addLog(`第 ${targetRun}/${totalRuns} 轮已无后续未用邮箱，本次自动运行结束。`, 'warn');
              forceFreshTabsNextRun = true;
              break;
            }
            ...
            if (blockedBySignupUserAlreadyExists) {
              ...
              const blockedState = await getState().catch(() => ({}));
              const blockedEmail = resolveCurrentEmailFromState(blockedState) || extractEmailFromText(reason);
              if (blockedEmail && typeof markCustomEmailPoolEntryUsedByEmail === 'function') {
                await markCustomEmailPoolEntryUsedByEmail(blockedEmail, {
                  state: blockedState,
                  logPrefix: 'iCloud API 邮箱池：检测到邮箱已注册/已有会话，',
                  level: 'warn',
                }).catch(() => null);
              }
              ...
              if (hasNextUnusedCustomEmail) {
                totalRuns += 1;
                roundSummaries.push(createAutoRunRoundSummary(totalRuns));
                runtime.set({ autoRunTotalRuns: totalRuns });
                await setState({
                  autoRunRoundSummaries: serializeAutoRunRoundSummaries(totalRuns, roundSummaries),
                  ...getAutoRunStatusPayload('running', { currentRun: targetRun, totalRuns, attemptRun, sessionId }),
                });
                await addLog(`第 ${targetRun}/${totalRuns} 轮邮箱已注册/已有会话，检测到邮箱池仍有未用邮箱，已自动追加 1 轮补位，继续等待目标全部成功。`, 'warn');
              }

              await addLog(
                `第 ${targetRun}/${totalRuns} 轮触发 user_already_exists/用户已存在${blockedEmail ? `（${blockedEmail}）` : ''}，本轮将直接失败并跳过剩余重试。`,
                'warn'
              );
              await addLog(
                targetRun < totalRuns
                  ? `第 ${targetRun}/${totalRuns} 轮因邮箱已注册/已有会话提前结束，自动换下一个未用邮箱继续。`
                  : `第 ${targetRun}/${totalRuns} 轮因邮箱已注册/已有会话提前结束，已无后续未用邮箱，本次自动运行结束。`,
                'warn'
              );
              forceFreshTabsNextRun = true;
              break;
            ...
            if (blockedByBadEmailCredential) {
              roundSummary.status = 'failed';
              roundSummary.finalFailureReason = reason;
              await setState({
                autoRunRoundSummaries: serializeAutoRunRoundSummaries(totalRuns, roundSummaries),
              ...
              await appendRoundRecordIfNeeded('failed', reason, err);
              cancelPendingCommands('当前轮因邮箱取码接口鉴权失败已终止。');
              await broadcastStopToContentScripts();

              const blockedState = await getState().catch(() => ({}));
              const blockedEmail = String(err?.email || '').trim().toLowerCase()
                || resolveCurrentEmailFromState(blockedState)
                || extractEmailFromText(reason);
              if (blockedEmail && typeof markCustomEmailPoolEntryUsedByEmail === 'function') {
                await markCustomEmailPoolEntryUsedByEmail(blockedEmail, {
                  state: blockedState,
                  logPrefix: 'iCloud API 邮箱池：取码接口鉴权失败，',
                  level: 'warn',
                }).catch(() => null);
              }
              ...
              if (hasNextUnusedCustomEmail) {
                ...
                await addLog(`第 ${targetRun}/${totalRuns} 轮邮箱取码凭证不可用，检测到邮箱池仍有未用邮箱，已自动追加 1 轮补位。`, 'warn');
              }

              await addLog(
                blockedEmail
                  ? `第 ${targetRun}/${totalRuns} 轮邮箱 ${blockedEmail} 的淘宝版查询码/接口凭证不可用，本轮跳过。`
                  : `第 ${targetRun}/${totalRuns} 轮邮箱取码接口鉴权失败，本轮跳过。`,
                'warn'
              );
              await addLog(
                targetRun < totalRuns
                  ? `第 ${targetRun}/${totalRuns} 轮将自动换下一个未用邮箱继续。`
                  : `第 ${targetRun}/${totalRuns} 轮已无后续未用邮箱，本次自动运行结束。`,
                'warn'
              );
              forceFreshTabsNextRun = true;
              break;
